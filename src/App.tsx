...
import { Room, Participant, HistoryState } from "./types";
import { 
  parseCSV, 
  ...
  SAMPLE_RETREAT_ROOMS_CSV,
  SAMPLE_RETREAT_REGISTRATION_CSV 
} from "./utils";
...
export default function App() {
  ...
  const handleResetAllocations = () => {
    const confirmation = window.confirm("Are you sure you want to clear all current bed assignments? Participants details will remain, but everyone will return to the unassigned list.");
  ...
  const handleAutoAllocate = () => {
    ...
    for (const participant of updatedParticipants) {
      ...
      for (const room of updatedRooms) {
        const roomMatchesPref = room.category.trim().toLowerCase() === participant.requestedRoomType.trim().toLowerCase();
    ...
    if (matchesCount > 0) {
      commitWorkspaceState(updatedRooms, updatedParticipants);
      alert(`Auto-allocation complete! Automatically assigned ${matchesCount} unassigned guests matching their exact requested room type and bed configuration.`);
    } else {
      alert("No additional unassigned participants could be auto-allocated. There are either no vacant beds remaining, or the available empty slots do not match any unassigned guest preferences!");
    }
  };
  ...
  return (
    <div id="application-container" className="min-h-screen bg-[#F8FAFC] text-slate-800 flex flex-col font-sans">
      ...
      {/* 
        ...
        ==================================================== 
      */}
      <header id="application-header" className="bg-slate-900 text-white border-b border-slate-950 px-6 py-4 sticky top-0 z-40 print:hidden shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button
              id="reload-demo-btn"
              onClick={loadDefaultRetreatDataset}
              className="px-3 py-2 bg-slate-800 hover:bg-slate-750 text-slate-350 text-xs font-semibold rounded-lg border border-slate-700 transition-all cursor-pointer flex items-center gap-1.5"
              title="Reset everything to starting demo parameters"
            >
              Reset Demo
            </button>
          </div>
      ...
      {/* 
        ====================================================
        NAVIGATION BAR (Hides automatically during printing)
        ==================================================== 
      */}
      <nav id="workspace-navigation" className="bg-white border-b border-slate-200 sticky top-[73px] md:top-[68px] z-30 print:hidden shadow-xs">
      ...
      */}
      <main id="workspace-core-view" className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 print:p-0">
        ...
        {/* --- VIEW 1: INTERACTIVE ALLOCATION BOARD --- */}
        {activeTab === "board" && (
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
            
            {/* Left Column: Registered Guest pool (Draggables) */}
            ...
            <div className="xl:col-span-8 space-y-6">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:hidden">
                ...
                <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                  <button
                    onClick={handleAutoAllocate}
                    className="flex-1 sm:flex-none px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold rounded-lg shadow-sm transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                  ...
                  <button
                    onClick={handleResetAllocations}
                    className="flex-1 sm:flex-none px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-extrabold rounded-lg border border-slate-200 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    Reset Assignments
                  </button>
                </div>
              ...
              {assignError && (
                <div id="allocation-error-banner" className="bg-rose-50 border-2 border-rose-200 text-rose-900 rounded-xl p-4 flex items-start gap-3 relative animate-fadeIn shadow-xs">
              ...
              {rooms.length === 0 ? (
                <div className="py-24 px-4 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-white flex flex-col items-center justify-center">
                  ...
                  <p className="text-xs text-slate-400 max-w-sm mb-6 leading-relaxed">
                    To start distributing attendees on beds, set up your Event Rooms spreadsheet list inside the "Raw CSV Setup" tab.
                  </p>
                  <button
                    onClick={() => setActiveTab("csv")}
